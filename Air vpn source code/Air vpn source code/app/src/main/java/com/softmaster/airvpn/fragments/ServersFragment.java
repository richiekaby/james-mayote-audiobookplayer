...
import com.softmaster.airvpn.ImagesUtil;
...
public class ServersFragment extends DialogFragment implements PassServerData {
    ...
    //lists
    private final List<Server> freeList = new ArrayList<>();
    private final List<Server> paidList = new ArrayList<>();
    ...
    private ArrayList getServerList() {
        ...
        servers.add(new Server("United States",
                ImagesUtil.getImgURL(R.drawable.usa_flag),
                "us.ovpn",
                "freeopenvpn",
                "416248023"
        ));
        servers.add(new Server("Japan",
                ImagesUtil.getImgURL(R.drawable.japan),
                "japan.ovpn",
                "vpn",
                "vpn"
        ));
        servers.add(new Server("Sweden",
                ImagesUtil.getImgURL(R.drawable.sweden),
                "sweden.ovpn",
                "vpn",
                "vpn"
        ));
        servers.add(new Server("Korea",
                ImagesUtil.getImgURL(R.drawable.korea),
                "korea.ovpn",
                "vpn",
                "vpn"
        ));

        paidList.addAll(servers);
        freeList.addAll(servers);
...
//    }
    ...
    @Override
    public void getSelectedServer(Server country) {
        SharedPreference preference = new SharedPreference(requireContext());
        preference.saveServer(country);
    ...
    public interface RegionChooserInterface {
        void onRegionSelected(Server item);
    }
}
